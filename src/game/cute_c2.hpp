/*
        ...
        THE IMPORTANT PARTS
                ...
                * c2MakePoly         - Runs convex hull algorithm and computes normals on input
   point-set
                ...
                * c2CastRay          - generic version of c2Rayto*** funcs
                ...
                Here is a very nice repo containing various tests and examples using SFML for
   rendering: https://github.com/sro5h/tinyc2-tests
...
CUTE_C2_API float c2GJK(
    const void *A, C2_TYPE typeA, const c2x *ax_ptr, const void *B, C2_TYPE typeB, const c2x *bx_ptr, c2v *outA, c2v *outB, int use_radius, int *iterations, c2GJKCache *cache);
...
CUTE_C2_API float c2TOI(
    const void *A, C2_TYPE typeA, const c2x *ax_ptr, c2v vA, const void *B, C2_TYPE typeB, const c2x *bx_ptr, c2v vB, int use_radius, c2v *out_normal, c2v *out_contact_point, int *iterations);
...
CUTE_C2_API int c2Collided(
    const void *A, const c2x *ax, C2_TYPE typeA, const void *B, const c2x *bx, C2_TYPE typeB);
CUTE_C2_API void c2Collide(
    const void *A, const c2x *ax, C2_TYPE typeA, const void *B, const c2x *bx, C2_TYPE typeB, c2Manifold *m);
...
C2_INLINE float c2Dot(c2v a, c2v b)
{
...
C2_INLINE c2v c2Div(c2v a, float b)
{
...
C2_INLINE float c2Det2(c2v a, c2v b)
{
...
C2_INLINE c2v c2Absv(c2v a)
{
    return c2V(c2Abs(a.x), c2Abs(a.y));
}
C2_INLINE float c2Hmin(c2v a)
{
    return c2Min(a.x, a.y);
}
C2_INLINE float c2Hmax(c2v a)
{
    return c2Max(a.x, a.y);
}
C2_INLINE float c2Len(c2v a)
{
    return c2Sqrt(c2Dot(a, a));
}
C2_INLINE c2v c2Norm(c2v a)
{
...
C2_INLINE c2v c2RotX(c2r r)
{
    return c2V(r.c, r.s);
}
C2_INLINE c2v c2RotY(c2r r)
{
    return c2V(-r.s, r.c);
}
...
C2_INLINE c2v c2Mulxv(c2x a, c2v b)
{
    return c2Add(c2Mulrv(a.r, b), a.p);
}
C2_INLINE c2v c2MulxvT(c2x a, c2v b)
{
...
// halfspace ops
C2_INLINE c2v c2Origin(c2h h)
{
    return c2Mulvs(h.n, h.d);
}
C2_INLINE float c2Dist(c2h h, c2v p)
{
    ...
    const void *A, C2_TYPE typeA, const c2x *ax_ptr, const void *B, C2_TYPE typeB, const c2x *bx_ptr, c2v *outA, c2v *outB, int use_radius, int *iterations, c2GJKCache *cache)
{
    ...
    // Metric and caching system as designed by E. Catto in Box2D for his conservative
    // advancment/bilateral advancement algorithim implementations. The purpose is to reuse old
    // simplex indices (any simplex that have not degenerated into a line or point) as a starting
    // point. This skips the first few iterations of GJK going from point, to line, to triangle,
    // lowering convergence rates dramatically for temporally coherent cases (such as in time of
    // impact searches).
...
static C2_INLINE float c2Step(
    float t, const void *A, C2_TYPE typeA, const c2x *ax_ptr, c2v vA, c2v *a, const void *B, C2_TYPE typeB, const c2x *bx_ptr, c2v vB, c2v *b, int use_radius, c2GJKCache *cache)
{
    c2x ax = *ax_ptr;
    c2x bx = *bx_ptr;
...
c2Incident(c2v *incident, const c2Poly *ip, c2x ix, const c2Poly *rp, c2x rx, int re)
{
...
void c2PolytoPolyManifold(
    const c2Poly *A, const c2x *ax_ptr, const c2Poly *B, const c2x *bx_ptr, c2Manifold *m)
{
    m->count = 0;
